...
export default function AuditTrailsPage() {
    ...
    const fetchLogs = async () => {
      try {
        ...
        const mappedLogs = data.map(log => ({
          ...
          component: "AI Engine Service",
          details: `Processed using ${log.modelUsed || 'default'}. Encounter: ${log.encounterId || 'N/A'}. Latency: ${log.latencyMs || 0}ms`,
          status: "Completed",
          statusColor: "success",
  ...
  const filteredLogs = logs.filter((log) => {
    ...
    return matchesSearch && matchesAction && matchesComponent && matchesUser;
  });
  ...
  React.useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, actionFilter, componentFilter, userFilter]);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Filters */}
      <Card padding="md" className="flex items-end gap-4">
        ...
        <div className="flex-1">
          <Input
            ...
            value={componentFilter === "All Components" ? "" : componentFilter}
            onChange={(e) =>
              setComponentFilter(e.target.value || "All Components")
            }
          />
        </div>
        <div className="w-48">
          ...
          <>
            ...
            <thead>
              <tr className="border-b border-neutral-500 bg-neutral-100">
                <th className="text-left px-4 py-3 text-xs font-medium text-neutral-800">
                  Timestamp
                </th>
                <th className="text-left px-4 py-3 text-xs font-medium text-neutral-800">
                  User
                ...
                <th className="text-right px-4 py-3 text-xs font-medium text-neutral-800"></th>
              </tr>
            </thead>
            <tbody>
              {pagedLogs.map((log) => {
                ...
                return (
                  <React.Fragment key={log.id}>
                    <tr
                      className={`border-b border-neutral-400 hover:bg-neutral-50 transition-colors cursor-pointer ${
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div
                          className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-2 text-xs font-medium ${config.color}`}
                        >
                      ...
                      <td className="px-4 py-3">
                        <StatusBadge
                          status={getStatusColor(log.status)}
                          label={log.status}
                        />
                      </td>
                      <td className="px-4 py-3 text-right">
                        <button className="p-1.5 rounded-2 hover:bg-neutral-200 transition-colors">
                          <ChevronDown
