...
export default function ProtocolCard({ protocol, isSelected, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left p-3 rounded-6 border transition-all group ${
        ...
        isSelected
          ...
          : "border-l-4 border-l-transparent bg-white dark:bg-neutral-800/50 hover:bg-neutral-50 dark:hover:bg-neutral-800 border-neutral-200 dark:border-neutral-700"
      }`}
    >
      <div className="flex items-center justify-between mb-1.5">
        <div className="flex items-center gap-2">
          ...
          {protocol.version > 1 && (
            <span className="text-[10px] font-bold text-neutral-500 dark:text-neutral-400">v{protocol.version}</span>
          )}
          {protocol.vectorized && (
            <span className="flex items-center gap-0.5 text-[10px] text-primary-600 dark:text-primary-400 font-semibold">
              <Brain className="w-2.5 h-2.5" /> RAG
            </span>
          )}
          {expiringSoon && (
            <span className="flex items-center gap-0.5 text-[10px] text-warning-500 font-semibold">
      ...
      <h4 className="text-xs font-semibold text-neutral-900 dark:text-neutral-200 leading-5 text-left">
        {protocol.title}
      </h4>
